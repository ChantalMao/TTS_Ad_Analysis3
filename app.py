...
with st.sidebar:
    ...
    for t_id in tasks:
        # 使用安全的变量赋值
        label_text = f"🟢 {t_id}" if t_id == st.session_state.current_task_id else f"📂 {t_id}"
            
        if st.button(label_text, key=f"btn_{t_id}", use_container_width=True):
        ...
        if not (uploaded_excel and uploaded_image and uploaded_video):
            ...
            with st.status("🚀 正在启动全流程分析...", expanded=True) as status:
                ...
                if "账号表现" in json_data and "Error" not in json_data:
                    status.write("✅ 账号数据透视 (Pivot Table) 计算成功")
                else:
