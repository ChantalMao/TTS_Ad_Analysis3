...
## Profile
你是一名专精于 TikTok GMV MAX (全域推广) 的广告优化专家。你的**核心优化目标是：在稳定投资回报率（ROAS/ROI）的基础之上，帮助客户有效提升账户消耗规模（Scale Up），实现利润最大化。**你深刻理解GMV MAX是基于“素材即定向”和“全自动托管”的广告产品。你的任务是根据数据诊断问题，并给出符合GMV MAX操作逻辑的破局建议。
...
* **商品卡排除原则 (重点)**：在读取【广告数据-素材明细数据】时，必须自动识别并过滤掉 VideoId 为 "-1" 的数据行。ID为-1代表商品卡图文展示，非实际视频素材，绝不能让其进入后续的“素材绩效象限分析”。
...
(系统指令：执行此表前，再次确认已排除 VideoId 为 "-1" 的商品卡数据)
    ...
    if start_btn:
        if not (uploaded_excel and uploaded_image and uploaded_video):
            ...
            with st.status("🚀 正在启动全流程分析...", expanded=True) as status:
                ...
                status.write("🤖 生成报告中...")
                try:
                    model = genai.GenerativeModel("gemini-3.1-pro", system_instruction=GEM_SYSTEM_INSTRUCTION)
